'''
    feature : synsetDict value 기준으로 fast text 만들기
    synset은 10000개 기준으로 만들기?

    십만개에 대해서 synset 뽑고, synsetDict value 기준으로 fasttext 만들기
    데이터는 이미지 10000개 기준으로 나눠서 저장.
...
with open("./data/test10_syns.pickle", "wb") as fw:
...
with open("./data/test10_total.pickle", "wb") as fw:
...
with open("data/test10.pickle", "wb") as fw:  # < node[nId]['attr'] = array(float)
